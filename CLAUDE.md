...
To switch tasks: edit those two USER CODE blocks. **Never enable two motor-driving tasks simultaneously.**
...
USER/
  ...
  task/
    main_tasks/
      inc/      Main (competition) task headers
      src/      Main task implementations — composite behaviors (follow+avoid, etc.)
    test_tasks/
...
3. **Test tasks & Main tasks**: Each experiment has a `test_xxx.c`/`.h` pair (in `test_tasks/`) or a main task (in `main_tasks/`) with `Xxx_Init()` and `Xxx_Tick()`. Enable an experiment by uncommenting its Init/Tick calls in `main.c` and `stm32f1xx_it.c`. `test_tasks/` are for single-driver debugging; `main_tasks/` are competition-level composite behaviors. **Only one task should own the motors at a time** — enabling multiple motor-driving tasks simultaneously causes conflicts.
...
1. Create `USER/driver/inc/xxx.h` and `USER/driver/src/xxx.c` — implement `Xxx_Init()` and `Xxx_Tick()`
2. Create `USER/config/xxx_config.h` — all tunable parameters as `#define` macros; driver `#include`s this
3. If it's a test task: create `USER/task/test_tasks/inc/test_xxx.h` and `USER/task/test_tasks/src/test_xxx.c`
   If it's a main (competition) task: create under `USER/task/main_tasks/`
4. Add `#include` in `main.c` (`USER CODE BEGIN Includes`)
5. Add `#include` in `stm32f1xx_it.c` (`USER CODE BEGIN Includes`)
6. Enable Init/Tick calls in the two USER CODE blocks (see "Enabling / Disabling Experiments" below)
7. No need to edit `CMakeLists.txt` — `file(GLOB_RECURSE ...)` auto-discovers new files under `USER/`
...
⚠️ Never enable two motor-driving tasks simultaneously (e.g. obstacle avoid + IR remote, or follow_avoid + line follow).
